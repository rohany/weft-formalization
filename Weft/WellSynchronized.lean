...
/-! ### §4.2 — The well-synchronization check (Figure 4)

`CheckWellSynchronized` is the executable algorithm of Figure 4 (`WELLSYNC`):
given a CTA `T` and *one* concrete complete trace `τ` that runs `(I, T)` to `done`
(Figure 4's `assume τ ≡ (I, T), …, (F, done)`), it returns a `Bool` that is `true`
iff `T` is well-synchronized (Definition 6).
...
This is the *checking* side of the development; the `*.WellSynchronized`
predicates above are the *specification* (Definitions 6–7). We implement the
algorithm faithfully but do not prove the paper's soundness/completeness
...
`recycleCount` counts recyclings directly from the trace, and the time `t(τ, η)`
is found by `pointTime`. We locate the executing step of `η = ⟨i, k⟩` by program
...
remaining length falls from `|T.prog i| − k` to `|T.prog i| − k − 1` — for a
`sync` this is the recycle step, exactly as in Definition 3. This matches
`IsTimeOf`/`IsGenOf` on a genuine trace from `(I, T)` (the 1-indexed `+ 1`
convention, with `0` for a command that never executes — see the `IsGenOf` doc). -/
...
section doc): the step from `C` to `C'` runs `η` exactly when `C` still has the
...
/-- **Figure 4 (`WELLSYNC`).** `CheckWellSynchronized T τ = true` iff the CTA `T` is
well-synchronized, where `τ` is a concrete complete trace from `(I, T)` ending in
`done` (the algorithm's standing assumption — e.g. from
`CTA.WellSynchronized.exists_successfulTrace`, or a concrete simulation; a `τ` that
deadlocks or errors already witnesses a violation and need not be checked).
...
generation `k+1` whose in-thread predecessor is `c3`, the closure must already order
`c1` before `c3` (`(c1, c3) ∈ Rstar`). If some such ordering is missing, a different
schedule could pair the synchronizations up differently, so return `false`. -/
def CheckWellSynchronized (T : CTA) (τ : List Config) : Bool :=
  -- Step 1 (lines 4–16): initialize R from the barrier generation counts.
  let R : Finset (ProgPoint × ProgPoint) := initRelation T τ
  -- Step 2 (line 17): take the transitive closure of R.
  let Rstar : Finset (ProgPoint × ProgPoint) := transClosure R
        ...
        T.progPoints.all fun c2 =>
          ...
          | some b' =>
              if b = b' ∧ pointGen T τ c2 = k + 1 ∧ 1 ≤ c2.idx then
                ...
                decide ((c1, (⟨c2.thread, c2.idx - 1⟩ : ProgPoint)) ∈ Rstar)
              else
                true
          | none => true
    | _ => true
